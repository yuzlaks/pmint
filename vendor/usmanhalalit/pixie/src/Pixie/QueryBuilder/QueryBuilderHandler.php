...
class QueryBuilderHandler
{
    ...
    public function paginate($limit, $params = 'page')
    {
        $pages = new Paginator($limit, $params);

        $pages->set_total($this->count()); 

        $this->limit($pages->get_limit_raw()[1])->offset($pages->get_limit_raw()[0]);

        $this->page_links = $pages->page_links();
